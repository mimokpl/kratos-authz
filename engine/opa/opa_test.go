...
import (
	...
	"github.com/mimokpl/kratos-authz/engine"
	"github.com/mimokpl/kratos-authz/engine/opa"
)
...
func TestAuthorizedWithStatements(t *testing.T) {
  ...
  "policies": {
	...
	cases := map[string]map[string]interface{}{
		"exact match": {
			"subjects": engine.MakeSubjects("team:local:admins"),
...
func TestAuthorizedProjects(t *testing.T) {
  ...
  "policies": {
	...
	cases := map[string]map[string]interface{}{
		"exact match": {
			"subjects": engine.MakeSubjects("team:local:admins"),
			"projects": engine.MakeProjects("p1", "p3", "p4"),
			"action":   "iam:teams:create",
			"resource": "iam:teams",
		},
	}
...
func TestHasProject(t *testing.T) {
	...
	input := map[string]interface{}{"projects": engine.MakeProjects(engine.Project(project))}
